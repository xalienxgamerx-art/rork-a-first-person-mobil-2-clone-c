...
class Renderer3D(val w: Int, val h: Int) : FrameRenderer {
    ...
    /** One thing to draw as a camera-facing billboard. */
    private class Board(val sprite: Sprite, val x: Float, val y: Float, val height: Float, val flash: Float)
    ...
    private fun drawSprites(scene: RenderScene) {
        ...
        scene.groundItems.forEach { drop ->
            if (dist2(drop.x, drop.y, camX, camY) <= cull) {
                boards += Board(Sprites[Sprites.ITEM], drop.x, drop.y, 0.22f, 0f)
            }
        }
        ...
        boards.forEach { board ->
            ...
            // Every soul stands on the land: the billboard's foot at the ground's height.
            val base = scene.map.heightAt(board.x, board.y)
